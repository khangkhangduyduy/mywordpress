...
function backuply_page_header($title = 'Settings') {
	...
	wp_localize_script('backuply-js', 'backuply_obj', array(
		...
		'creating_session' => wp_generate_password(32)
	));
...
function backuply_page_backup(){
	...
	if(isset($_REQUEST['backuply_edit_location']) && array_key_exists($edit_loc_id, (array)$backuply_remote_backup_locs)){
		...
		if($protocol == 'dropbox' || $protocol == 'gdrive' || $protocol == 'aws' || $protocol == 'onedrive'){
		...
		if($protocol != 'dropbox' && $protocol != 'gdrive' && $protocol != 'webdav' && $protocol != 'aws' && $protocol != 'onedrive'){
	...
	if(isset($_REQUEST['addbackuploc'])){
		...
		if($protocol == 'dropbox' || $protocol == 'gdrive' || $protocol == 'aws' || $protocol == 'onedrive'){
			...
			if($protocol == 'onedrive'){
				...
				$aws = backuply_load_remote_backup('aws');
							...
							<form method="post">
								...
								<td>
									<div style="display:flex; align-items:center;">
										<img src="<?php echo BACKUPLY_URL . '/assets/images/'.esc_attr($v['protocol']).'.svg'; ?>" height="26" width="26"/> &nbsp;<?php echo esc_html($protocols[$v['protocol']]); ?>
									</div>
								</td>
					...
					<?php if(!defined('BACKUPLY_PRO')) { ?>
						<div class="backuply-settings-block backuply-premium-box">
							...
							<ul class="backuply-loc-types-showcase">
								...
								<li><img class="backuply-grayscale" src="<?php echo BACKUPLY_URL . '/assets/images/gdrive.svg'?>" height="64" width="64" alt="Google Drive Logo" title="Google Drive"/></li>
		...
		<div class="inside">
			<form method="post">	
				...
				<div class="backuply-option-wrap aws_s3bucket">
					...
					<input type="text" size="50" name="aws_secretKey" id="aws_secretKey" value="<?php echo esc_attr($aws_secret_key); ?>"/>
				</div>
				<div class="backuply-option-wrap aws_s3bucket">
					...
					foreach($backup_infos as $count => $all_info):
						...
						echo '
							...
							<td>
								<div style="display:flex; align-items:center;">';
									if(!empty($all_info->backup_location)) {
										echo '<img src="'.BACKUPLY_URL.'/assets/images/'.esc_attr($backup_protocol).'.svg" height="26" width="26" alt="Remote Location: '.esc_attr($backup_protocol).'"/ > &nbsp; ';
									}
