...
jQuery(document).ready(function(){
	...
	jQuery('.backuply-download-backup').click(function(){
		...
		let file_name = jQuery(this).data('name'),
			dir = jQuery(this).data('path')
		
		backuply_download_backup(file_name, dir);
	});
	...
	jQuery('.backuply-backup-nag .notice-dismiss').click(function(){
		...
		jQuery.ajax({
			method : 'GET',
			url : backuply_obj.ajax_url + '?action=backuply_hide_backup_nag&security='+backuply_obj.nonce,
			success : function(res){
				console.log(res);
			}
		})
...
function backuply_download_backup(file_name, dir) {
	jQuery.ajax({
		method : 'POST',
		url : backuply_obj.ajax_url,
		xhrFields : {
		...
		success : function(res) {
			...
			let a = document.createElement('a'),
				url = URL.createObjectURL(new Blob([res], {type : 'application/x-tar'}));
